"""TechLife Home Assistant Integration"""
import logging

import homeassistant.helpers.config_validation as cv
import paho.mqtt.client as mqtt
import voluptuous as vol
from homeassistant.components.light import (SUPPORT_BRIGHTNESS, ATTR_BRIGHTNESS, PLATFORM_SCHEMA, LightEntity)
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Required(CONF_MQTT_URL): cv.string,
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    """Set up the Techlife Light platform."""
    # Assign configuration variables.
    # The configuration check takes care they are present.
    mac = config.get(CONF_MAC_ADDRESS)
    name = config.get(CONF_NAME)
    mqtt_url = config.get(CONF_MQTT_URL)
...
class TechlifeControl(LightEntity):
    """Representation of a Techlife Light."""

    def __init__(self, mac, client, name):
        """Initialize an Techlife light."""
        self.mac = mac
        self.client = client
        self._name = name
        self._state = False
    ...
    def turn_on(self, **kwargs):
        ...
        You can skip the brightness part if your light does not support brightness control.
        """
        if not self._state:
            self.on()
        self._state = True
